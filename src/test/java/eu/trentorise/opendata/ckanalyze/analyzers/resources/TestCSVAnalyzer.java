...
public class TestCSVAnalyzer {
	...
	public void analyzeEmptyFile()
	{
		try
		{
			CSVAnalyzer csva = new CSVAnalyzer(ClassLoader.getSystemResource("csv/empty.csv").getPath(),"");
			csva.analyze();
			assertTrue(false);
		}catch(CKAnalyzeException e)
		{
			assertTrue("must throw an exception since the resource is empty",e.toString().contains("Empty resource"));			
		}		
	}
	...
	public void analyzeHeadersOnlyCSV()
	{
		try
		{
			CSVAnalyzer csva = new CSVAnalyzer(ClassLoader.getSystemResource("csv/headers_only.csv").getPath(),"");
			csva.analyze();
			assertTrue(false);
		}catch(CKAnalyzeException e)
		{
	...
	public void analyzeMalformedCSV()
	{
		...
		WriterAppender appender = new WriterAppender(new PatternLayout("%d{ISO8601} %p - %m%n"),toCheck);
		appender.setName("LOGGER_APPENDER");
		Logger.getRootLogger().addAppender(appender);
		appender.setThreshold(org.apache.log4j.Level.WARN);
		try
		{
			CSVAnalyzer csva = new CSVAnalyzer(ClassLoader.getSystemResource("csv/malformed.csv").getPath(),"");
			csva.analyze();
			assertTrue("must report a warning message since the CSV is malformed",!toCheck.toString().isEmpty());
		}catch(CKAnalyzeException e)	{	
			assertTrue(false);
		}
	}
	...
	public void analyzeProdottiTradizionali()
	{
		...
		try
		{
			CSVAnalyzer csva = new CSVAnalyzer(ClassLoader.getSystemResource("csv/prodotti_tradizionali.csv").getPath(),"");
			csva.analyze();
			assertTrue("must process the CSV prodotti_tradizionali without errors or warnings",toCheck.toString().isEmpty());
		}catch(CKAnalyzeException e)	{	
			assertTrue(false);
		}
	}
	...
	public void analyzeResourceWithTypeIdentification()
	{
		...
		try
		{
			CSVAnalyzer csva = new CSVAnalyzer(ClassLoader.getSystemResource("csv/_XNz0Y0.csv").getPath(),"");
			csva.analyze();
			assertTrue("must process the CSV without errors or warnings",toCheck.toString().isEmpty());
